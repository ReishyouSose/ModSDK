...
namespace Assets.THCompass.DropManager.Rule
{
    public abstract class DropRule
    {
        ...
        public abstract IEnumerable<DropTableInfo> SelfToInfo();
        ...
        public List<DropTableInfo> ToDropTableInfo()
        {
            List<DropTableInfo> list = new();
            ToDropTableInfo(list);
            return list;
        }
        private void ToDropTableInfo(List<DropTableInfo> list)
        {
            foreach (var info in SelfToInfo())
            {
                list.Add(info);
            }
            SuccessChain.ForEach(x => x.ToDropTableInfo(list));
            FailureChain.ForEach(x => x.ToDropTableInfo(list));
        }
